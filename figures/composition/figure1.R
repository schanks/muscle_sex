...
datmelt=melt(dat[,-c(2,15)], id.vars="labelcode")
...
b=ggplot(datmelt, aes(x=labelcode, fill=`Cell type`, y=Proportion))+geom_bar(position="stack",stat="identity", width=1)+theme_bw()+xlab(" ")+theme(axis.text.x=element_text(color="white"),axis.ticks.x=element_blank(), axis.text.y=element_text(size=7), legend.text=element_text(size=7), legend.title=element_text(size=7),axis.title=element_text(size=7))+scale_y_continuous(expand=c(0,0))+scale_fill_manual(values=colorsmfm)+geom_text(label="Female:\nn=117", size=3,aes(x=5,y=0.94), check_overlap=TRUE, hjust=0, color="white")+geom_text(label="Male:\nn=162", size=3,aes(x=125,y=0.94), color="white",check_overlap=TRUE, hjust=0)+guides(fill=guide_legend(keywidth=0.7, keyheight=0.7))
...
d=ggplot(dat, aes(y=celltype, x=OR, color=sig))+geom_vline(xintercept=1, linetype="longdash")+geom_point(size=1)+geom_errorbar(aes(xmin=LB, xmax=UB))+scale_y_discrete(limits=rev)+theme_bw()+scale_color_manual(values=c("#e41a1c","#377eb8","black"))+theme(legend.position="none", axis.title.x=element_text(size=7), axis.text.x=element_text(size=7), axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank())+ylab("Cell type")+xlab("Fold change of male to female nuclei counts")+scale_x_continuous(trans=log_trans())
