...
nuc=nuc[which(nuc$modality=="rna"),]
...
dat=readRDS("/net/snowwhite/home/aujackso/sn_muscle_2023/data/df.rna.nuc.Rds")
dat[is.na(dat)]=0
...
means$textlabel[which(means$textlabel=="0")]="0.00"
c=ggplot(means, aes(x=`Mean proportion`, y=`Cell type`, fill=`Cell type`))+geom_bar(stat="identity")+scale_x_continuous(breaks=breaks, labels=labels, limits=c(-0.48,0.48))+scale_y_discrete(limits=rev)+theme_bw()+scale_fill_manual(values=colors1)+geom_text(aes(label=textlabel, x=textpos))+geom_text(label="Females", aes(x=-0.35, y=0.9), check_overlap=TRUE, size=4.5,hjust=0)+geom_vline(xintercept=0, size=0.2)+geom_text(label="Males", aes(x=.25, y=0.9), check_overlap=TRUE,size=4.5, hjust=0)+theme(legend.position="none", axis.text=element_text(size=12), axis.title=element_text(size=14))

#Panel D - Negative binomial results
dat=readRDS("/net/snowwhite/home/aujackso/sn_muscle_2023/output/nuclei_nb/nb_nuclei_invnorm_cell_SEX_fdr_across_celltypes_RNAnuc.Rds")
...
dat$celltype[which(dat$celltype=="Mesenchymal Stem Cell")]="Fibro-adipogenic progenitor"
dat$celltype=factor(dat$celltype, levels=c("Type 1", "Type 2a","Type 2x","Endothelial","Fibro-adipogenic progenitor","Smooth Muscle","T cell","Neuronal","Neuromuscular junction","Satellite Cell","Macrophage","Adipocyte"))
d=ggplot(dat, aes(y=celltype, x=OR, color=sig))+geom_vline(xintercept=1, linetype="longdash")+geom_point()+geom_errorbar(aes(xmin=LB, xmax=UB))+scale_y_discrete(limits=rev)+theme_bw()+scale_color_manual(values=c("#e41a1c","#377eb8","black"))+theme(legend.position="none", axis.title.x=element_text(size=14), axis.text.x=element_text(size=12), axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank())+ylab("Cell type")+xlab("Fold change of male to female nuclei counts")+scale_x_continuous(breaks=c(0.25,0.5,0.75,1.0,1.25,1.5,1.75,2.0),trans=log_trans())
