...
traits2<-c("SEX.M", "AGE", "T2D", "INSFAST", "BMI", "GLUFAST", "HEIGHT", "WEIGHT", "WAIST", "WHR", "HIP", "ALAT", "CRP", "CHOL", "HDL", "LDL", "TG", "GL0", "GL30", "GL60", "GL120", "APOA1", "APOB", "CPEP", "CREAT", "GT", "URIC", "CPEP30", "S_INS30", "HBA1C", "GLU2H", "ENERGY", "PA_TOT", "PA_MOD", "PA_STREN", "INS0", "INS30", "INS60", "INS120", "RFM", "SBP", "DBP", "APOB.A1.RATIO", "INS_AUC_0_30", "GLU_AUC_0_30", "INSSEC30", "INSGENIN", "DI", "CPEPGENIN", "HOMA", "MATSUDA4", "MATSUDA3", "LTPA.DUR.ALL", "LTPA.DUR.COND", "LTPA.DUR.NONCOND", "LTPA.EN.ALL", "LTPA.EN.COND", "LTPA.EN.NONCOND", "LTPA.EN.LIGHT", "LTPA.EN.MOD", "LTPA.EN.MODVIG", "LTPA.EN.VIG", "SLEEP", "ALCOHOL", "SLEEP24H")   
	...
	if (traits[j] %in% c("SEX","AGE")) {
	 	pds<-pds %>% 
		dplyr::select(SEX, labelcode, AGE, area_name, tss_enrichment, batch, fraction_mitochondrial_median, tot_nuc_allcelltypes, ogtt_status_paper) %>% 
		filter(complete.cases(.)) 
	}
	
	if (!(traits[j] %in% c("SEX","AGE"))) {
	 	pds<-pds %>% 
		dplyr::select(traits[j], SEX, labelcode, AGE, area_name, tss_enrichment, batch, fraction_mitochondrial_median, tot_nuc_allcelltypes, ogtt_status_paper) %>% 
		filter(complete.cases(.)) 
	}
	...
	if (traits[j]=="SEX") {
		fstr<-"~ as.factor(batch) + as.factor(area_name) + invage + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) + as.factor(SEX)" 
		redstr<-"~ as.factor(batch) + as.factor(area_name) + invage + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) " 
	}
	if (traits[j]=="T2D") {
		fstr<-"~ as.factor(batch) + as.factor(area_name) + as.factor(SEX) + invage + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) + T2D"
		redstr<-"~ as.factor(batch) + as.factor(area_name) + as.factor(SEX) + invage + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) "
	}
	if (traits[j]=="AGE") {
		pds$x<- pds %>% pull(traits[j])
		pds$x<-invnorm(pds$x, j)
		fstr<-"~ as.factor(batch) + as.factor(area_name) + as.factor(SEX) + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) + x"
		redstr<-"~ as.factor(batch) + as.factor(area_name) + as.factor(SEX) + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) "
	}
	if (!(traits[j] %in% c("SEX", "AGE", "T2D"))) {
		pds$x<- pds %>% pull(traits[j])
		pds$x<-invnorm(pds$x, j)
		fstr<-"~ as.factor(batch) + as.factor(area_name) + as.factor(SEX) + invage + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) + x"
		redstr<-"~ as.factor(batch) + as.factor(area_name) + as.factor(SEX) + invage + invtssenrich + invmito + invtotnuc + as.factor(ogtt_status_paper) "
	}
...
write.table(format(res, digits=8), 
	file=paste0("~/sn_muscle_2023/output/DESeq.ATAC/final_drop10nuc/results_ogtt_covar/",celltype[i],".",traits2[j],".results.tab"), sep="\t", quote=F, col.names=T, row.names=F)
