...
traits2<-c("SEX.M", "AGE", "T2D", "INSFAST", "BMI", "GLUFAST", "HEIGHT", "WEIGHT", "WAIST", "WHR", "HIP", "ALAT", "CRP", "CHOL", "HDL", "LDL", "TG", "GL0", "GL30", "GL60", "GL120", "APOA1", "APOB", "CPEP", "CREAT", "GT", "URIC", "CPEP30", "S_INS30", "HBA1C", "GLU2H", "ENERGY", "PA_TOT", "PA_MOD", "PA_STREN", "INS0", "INS30", "INS60", "INS120", "RFM", "SBP", "DBP", "APOB.A1.RATIO", "INS_AUC_0_30", "GLU_AUC_0_30", "INSSEC30", "INSGENIN", "DI", "CPEPGENIN", "HOMA", "MATSUDA4", "MATSUDA3", "LTPA.DUR.ALL", "LTPA.DUR.COND", "LTPA.DUR.NONCOND", "LTPA.EN.ALL", "LTPA.EN.COND", "LTPA.EN.NONCOND", "LTPA.EN.LIGHT", "LTPA.EN.MOD", "LTPA.EN.MODVIG", "LTPA.EN.VIG", "SLEEP", "ALCOHOL", "SLEEP24H") 
	...
	if (traits[j] %in% c("SEX","AGE")) {
	 	pds<-pds %>% 
		dplyr::select(SEX, labelcode, AGE, med.frac.mito, area_name, snrna_batch, tot_nuc,Adipocyte, Endothelial, Macrophage, Mesenchymal_Stem_Cell,  Neuromuscular_junction, Neuronal, Satellite_Cell, Smooth_Muscle, T_cell, Type_1, Type_2a) %>% 
		dplyr::filter(complete.cases(.)) 
	}
	if (!(traits[j] %in% c("SEX","AGE"))) {
	 	pds<-pds %>% 
		dplyr::select(traits[j], SEX, labelcode, AGE, med.frac.mito, area_name, snrna_batch, tot_nuc,Adipocyte, Endothelial, Macrophage, Mesenchymal_Stem_Cell,  Neuromuscular_junction, Neuronal, Satellite_Cell, Smooth_Muscle, T_cell, Type_1, Type_2a)  %>% 
		dplyr::filter(complete.cases(.)) 
	}
	...
	if (traits[j]=="SEX") {
		fstr<-"~ as.factor(snrna_batch) + as.factor(area_name) + invage + invmito + invnucall + Adipocyte + Endothelial + Macrophage + Mesenchymal_Stem_Cell + Neuromuscular_junction + Neuronal + Satellite_Cell + Smooth_Muscle + T_cell + Type_1 + Type_2a + as.factor(SEX)" 
	}
	if (traits[j]=="T2D") {
		fstr<-"~ as.factor(snrna_batch) + as.factor(area_name) + as.factor(SEX) + invage + invmito + invnucall + Adipocyte + Endothelial + Macrophage + Mesenchymal_Stem_Cell +Neuromuscular_junction + Neuronal + Satellite_Cell + Smooth_Muscle + T_cell + Type_1 + Type_2a + T2D"
	}
	if (traits[j]=="AGE") {
		pds$x<- pds %>% pull(traits[j])
		pds$x<-invnorm(pds$x, j)
		fstr<-"~ as.factor(snrna_batch) + as.factor(area_name) +  as.factor(SEX) + invmito + invnucall + Adipocyte + Endothelial + Macrophage + Mesenchymal_Stem_Cell + Neuromuscular_junction + Neuronal + Satellite_Cell + Smooth_Muscle + T_cell + Type_1 + Type_2a + x"
	}
	if (!(traits[j] %in% c("SEX", "AGE", "T2D"))) {
		pds$x<- pds %>% pull(traits[j])
		pds$x<-invnorm(pds$x, j)
		fstr<-"~ as.factor(snrna_batch) + as.factor(area_name) + as.factor(SEX) + invage + invmito + invnucall + Adipocyte + Endothelial + Macrophage + Mesenchymal_Stem_Cell + Neuromuscular_junction + Neuronal + Satellite_Cell + Smooth_Muscle + T_cell + Type_1 + Type_2a + x"
	}
...
map<-read_tsv("~/sn_muscle_2023/data/gencode.v30.gene_lengths_tss.tsv") %>% 
 select(gene_id, gene_name, chrom, gene_start, gene_end, gene_strand, gene_type)

res1<-merge(res, map, by.x="gene", by.y="gene_id", all.x=T) %>% arrange(pvalue)

write.table(format(res1, digits=8), 
file=paste0("~/sn_muscle_2023/output/DESeq.RNA/pbulk_adjprop/pb.",traits2[j],".results.tab"), sep="\t", quote=F, col.names=T, row.names=F)
